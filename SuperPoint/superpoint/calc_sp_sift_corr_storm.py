...
if __name__ == '__main__':
    ...
    methods = ['nostyle','comir','unet_aligned','cyclegan_aligned','cut_aligned','pix2pix_aligned','cyclegan_unaligned','cut_unaligned','pix2pix_unaligned']
    # methods = ['nostyle','comir','cyclegan_aligned','cut_aligned','pix2pix_aligned','unet_aligned',]
    ...
    for method in methods:
        ...
        if method == 'pix2pix_aligned':
            p2p_fake_images = './datasets//pytorch-CycleGAN-and-pix2pix/results/p2p_storm647_aligned_selected/test_latest/images/*_fake_B.png' 
            p2p_path = './datasets//pytorch-CycleGAN-and-pix2pix/results/p2p_storm647_aligned_selected/test_latest/images/'
            images = glob.glob(f"{p2p_path}*_fake_B.png")
            # fix_p = f'{p2p_path}/{fname}{ext}'.replace("fake_B","real_B")
        elif method == 'cyclegan_aligned':
            p2p_fake_images = './datasets//pytorch-CycleGAN-and-pix2pix/results/cycle_dstrom_resize_mode_aligned/test_latest/images/*_fake.png'
            p2p_path = './datasets//pytorch-CycleGAN-and-pix2pix/results/p2p_storm647_aligned_selected/test_latest/images/'
            images = glob.glob(p2p_fake_images)
            # fix_p = f'{p2p_path}/{fname}{ext}'.replace("fake","real_B")
        elif method == 'comir':
            p2p_fake_images = './datasets//CoMIR/results/export_dstorm/*R2.tif'
            p2p_path = './datasets//CoMIR/results/export_dstorm/'
            images = glob.glob(p2p_fake_images)
            # fix_p = f'{p2p_path}/{fname}{ext}'.replace("fake","real_B")    
        elif method =='cut_aligned':
            p2p_fake_images = './datasets//contrastive-unpaired-translation/results/cut_d_storm_aligned_mode_aligned/test_latest/images/fake_B/*.png'
            p2p_path = './datasets//contrastive-unpaired-translation/results/cut_d_storm_aligned_mode_aligned/test_latest/images/real_B/'
            images = glob.glob(p2p_fake_images)
        elif method == 'unet_aligned':
            p2p_fake_images = '/storage01/grexai/dev/Unet-torch/test_config_dstrom/*.tiff'
            p2p_path = '/storage01/grexai/datasets/imgreg/Image_registration/dstrom/test/testB/'
            images = glob.glob(p2p_fake_images)
            # fix_p = f'{img2_file}/{fname}{ext}'
            # fix_p = f'{p2p_path}/{fname}{ext}'.replace("fake","real_B")
        elif method == 'pix2pix_unaligned':
            p2p_fake_images = './datasets//pytorch-CycleGAN-and-pix2pix/results/p2p_storm647_unaligned_selected/test_latest/images/*_fake_B.png' 
            p2p_path = './datasets//pytorch-CycleGAN-and-pix2pix/results/p2p_storm647_unaligned_selected/test_latest/images/'
            images = glob.glob(f"{p2p_path}*_fake_B.png")
            # fix_p = f'{p2p_path}/{fname}{ext}'.replace("fake_B","real_B")
        elif method == 'cyclegan_unaligned':
            p2p_fake_images = './datasets//pytorch-CycleGAN-and-pix2pix/results/cycle_dstorm_unaligned_selected/test_latest/images/*_fake.png'
            p2p_path = './datasets//pytorch-CycleGAN-and-pix2pix/results/p2p_storm647_unaligned_selected/test_latest/images/'
            images = glob.glob(p2p_fake_images)
            # fix_p = f'{p2p_path}/{fname}{ext}'.replace("fake","real_B")
        elif method =='cut_unaligned':
            p2p_fake_images = './datasets//contrastive-unpaired-translation/results/cut_dstorm_unaligned_resize_selected/train_latest/images/fake_B/*.png'
            p2p_path = './datasets//contrastive-unpaired-translation/results/cut_dstorm_unaligned_resize_selected/train_latest/images/real_B/'
            images = glob.glob(p2p_fake_images)    
        elif method == 'nostyle':
            p2p_fake_images  = '/storage01/grexai/datasets/imgreg/Image_registration/dstrom/test/testA/*.tiff' 
            p2p_path ='/storage01/grexai/datasets/imgreg/Image_registration/dstrom/test/testB/'
            images = glob.glob(p2p_fake_images)
        else:
            continue
        for image in tqdm(images):
            ...
            #check for pinv yes
            img1, img1_orig = preprocess_image(fix_p, img_size)
            img2, img2_orig = preprocess_image(image, img_size)
